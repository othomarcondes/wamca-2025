...
behaviors, with the 3\times2 and the 6\times1 data distributions having a mean
idle time of \approximate900ms and \aproximate1100ms respectively in
the first 10s of execution. Also, the ABE (Area Bound Estimation);
representing the optimal runtime, ignoring dependencies; which is
represented by a gray bar in each nodes bar, also show similar values,
indicate that the workload was distributed similarly between the nodes
in both executions.
