...
basic concepts on matrix distribution, modern task-based runtimes and
related work. Section [[sec:methodology]] details our methodology and
explains how we conducted the experiments in our
...
We use the LU factorization implementation of the Chameleon dense
...
cite:blackford1997scalapack, but follows the task-based
paradigm. Consequently, function calls are task submissions for a
runtime system. When programming for StarPU-MPI, the application code
must contain the static data partitioning decision, typically
following the block-cyclic distribution by default. The StarPU
heuristics carry out the task scheduling within a compute node
dynamically. Because of this hybrid mechanism, and the fact that
\verb|DGEMM| tasks are the most common kernel, it becomes mandatory to
have well distributed sub-matrixes between the nodes using a good tile
distribution.
