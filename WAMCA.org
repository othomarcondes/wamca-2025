...
** 2025-09-06 Amount of "MPI Communication" for each data partitioning strategy, etc
...
interesting phenomena, according to the changing block-cyclic
strategy, the scheduler and the transport layer.
#+latex: %
/Block-Cyclic Strategy/: We expected the 2\times3 and 3\times2 data partitions to
...
that the main reason for these results, different from what has been
observed in previous studies cite:nesi2020communication, is double
fold. Firstly, the workload is simply too small. Secondly, despite
being different, these data distributions
