...
linear algebra suite cite:chameleon. Its parallelization strategy is
very similar to that of ScaLAPACK cite:blackford1997scalapack (the
message passing version of LAPACK cite:anderson1999lapack), but
following the task-based paradigm where function calls are in fact
task submissions for a runtime system.  As shown in the Figure
...
kernels: \verb|DGTRF-NOPIV|, \verb|DTRSM| and \verb|DGEMM|. This
application has a tendency to be dominated by \verb|DGEMM| kernels
when $N$ is large, which makes it mandatory to have well distributed
sub-matrixes between the nodes. When programming for StarPU-MPI, the
application code carries out the data partititioning following the
block-cyclic distribution by default.
