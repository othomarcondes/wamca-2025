...
#    svz$config$st$idleness <- TRUE
  ...
  mutate(GC = map(STARVZ, panel_st)) |>
#  mutate(GC = map2(GC, TRANSPORT, function(gc, tr) { gc + ggtitle(tr) })) |>
  mutate(GC = map(GC, function(gc) { gc + xlim(0, 30000) })) |>
  arrange(pq) |>
...
wrap_plots(l.plots, nrow=2)
ggsave("img/lws-vs-dmdas-3x2-openmpi-traces.pdf", width=6, height=10)
#+end_src
...
#+RESULTS:
: `summarise()` has grouped output by 'TRANSPORT', 'size', 'block_size', 'scheduler'. You can override using the
: `.groups` argument.
: # A tibble: 2 × 8
...
wrap_plots(l.plots, nrow=2)
ggsave("img/lws-3x2-versus-6x1-openmpi-traces.pdf", width=6, height=10)
#+end_src
...
#+RESULTS:
: # A tibble: 2 × 7
:   TRANSPORT  size block_size scheduler pq    STARVZ     makespan
:   <chr>     <dbl>      <dbl> <chr>     <chr> <list>        <dbl>
: 1 openmpi   14400        360 lws       3x2   <strvz_dt>   21951.
: 2 openmpi   14400        360 lws       6x1   <strvz_dt>   22713.
...
Figure [[fig.trace3]] depicts two space/time views, each one with all the 144 workers of the six machines
(on the Y axis), considering the three types of tasks (colors). From
top to bottom, we represent the behavior of the =lws= and =dmdas= intra-node schedulers,
both using the 3\times2 data distribution. We can observe that both the executions have similar
behavior, with =dgemm= tasks representing the major part of the first 10s, and a mean idle time of
\approximate1100ms. Nevertheless,
...
*** Similarities on the =lws= executions

Figure [[fig.trace4]] depicts two space/time views, each one with all the 144 workers of the six machines
(on the Y axis), considering the three types of tasks (colors). From
top to bottom, we represent the behavior of the 3\times2 and 6\times1 data distributions,
both using the =lws= scheduler. We can observe that both the executions had similar behaviors,
with the 3\times2 and the 6\times1 data distributions having a mean
idle time of \approximate900ms and \aproximate1100ms respectively in the first 10s of execution. Also,
the ABE (Area Bound Estimation); representing the optimal runtime, ignoring dependencies; which is
represented by a gray bar in each nodes bar, also show similar values, indicate that the workload was distributed
similarly between the nodes in both executions.
