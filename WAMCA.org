...
The use of task-based runtime systems to execute parallel programs in a distributed way is adopted by various libraries and runtime systems such as StarPU, OmpSs and PaRSEC, providing programming models that abstract parallelism through tasks rather than explicit thread or process management. In these systems, computations are expressed as a set of tasks with declared data dependencies. At runtime, a scheduler dynamically builds a task dependency graph (DAG) and maps tasks to available resources (e.g., CPUs or GPUs). This approach relieves programmers from explicitly handling synchronization, communication and allowing an easy way to implement different data distributions.
...
behavior of =dgemm= tasks. However
