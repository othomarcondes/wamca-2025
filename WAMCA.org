...
*** Plot just one for the WAMCA presentation

#+begin_src R :results output :session *R* :exports both :noweb yes :colnames yes
l.plots[[2]] + theme(legend.position="top") -> p
ggsave("img/only_dmdas-3x2-openmpi-traces.pdf", width=12, height=6)
#+end_src
...
df2 |>
  mutate(STARVZ = map(STARVZ, function(svz) {
    ...
    svz$config$st$tasks$list <- c("4_2939", "4_2940")
    svz$config$st$tasks$levels <- 3
    svz$config$limits$start <- 0 #2750
    svz$config$st$alpha <- 0.1
#    svz$config$limits$end <- 3000
#    svz$config$base_size <- 20
    svz$config$st$aggregation$step <- 100
    svz$config$st$abe$active <- TRUE
    svz$config$st$abe$size <- 10
...
#    svz$config$st$idleness <- TRUE
    ...
    return(svz) })) |>
  mutate(STARVZ = map(STARVZ, function(svz) {
#    svz$Application |> filter(Node == 0) -> svz$Application
#    svz$Application |> filter(!Outlier) -> svz$Application
#    svz$Application |> mutate(Outlier = Duration > 90) -> svz$Application       
    svz
  })) |>
  mutate(GC = map(STARVZ, panel_st)) |>
#  mutate(GC = map2(GC, TRANSPORT, function(gc, tr) { gc + ggtitle(tr) })) |>
  ...
  arrange(pq) |>
  mutate(Label = paste0(scheduler, ", ", pq)) |>
  mutate(GC = map2(GC, Label, function(gc, p) { gc + labs(y = paste0(p, "\nApplication Workers")) })) -> df.plots
df.plots$GC[[1]]
df.plots$STARVZ[[1]]$Application |>
#  filter(Start > 2800) |>
#  filter(Node == 4) |>
  select(JobId, everything()) |>
  arrange(Start) |>
  filter(Value == "dgemm") |>
  print()# |>
#  distinct(Params)
#  pull(Duration) |> summary()
#  ggplot(aes(x = Iteration, y = Duration, color=Iteration)) + geom_point()
#+end_src
...
Adding another scale for x, which will replace the existing scale.
# A tibble: 20,540 × 24
   JobId  ResourceId Start   End Duration Depth Value    Size Params  
   <chr>  <chr>      <dbl> <dbl>    <dbl> <dbl> <fct>   <int> <fct>   
 1 0_1813 0_CPU12     53.5  57.3     3.79     0 dgemm 3110400 M360x36…
 2 0_1817 0_CPU21     53.7  62.5     8.81     0 dgemm 3110400 M360x36…
 3 0_1714 0_CPU14     57.6 175.    117.       0 dgemm 3110400 M360x36…
 4 0_1760 0_CPU12     57.6  66.6     8.92     0 dgemm 3110400 M360x36…
 5 0_1712 0_CPU20     57.6  68.5    10.8      0 dgemm 3110400 M360x36…
 6 0_1718 0_CPU19     57.7 180.    122.       0 dgemm 3110400 M360x36…
 7 0_1720 0_CPU17     57.7  72.6    14.9      0 dgemm 3110400 M360x36…
 8 0_1719 0_CPU18     57.7  74.7    17.0      0 dgemm 3110400 M360x36…
 9 0_1762 0_CPU13     57.7  76.8    19.1      0 dgemm 3110400 M360x36…
10 0_1949 0_CPU11     59.2  86.4    27.3      0 dgemm 3110400 M360x36…
# ℹ 20,530 more rows
# ℹ 15 more variables: Footprint <fct>, Tag <fct>, SubmitOrder <chr>,
#   GFlop <dbl>, X <int>, Y <int>, Iteration <int>,
#   Subiteration <int>, NumaNodes <chr>, Node <fct>, Resource <fct>,
#   ResourceType <fct>, Outlier <lgl>, Height <dbl>, Position <dbl>
# ℹ Use `print(n = ...)` to see more rows
#+end_example
